...
if ($action === "ping") {
    ...
    $host = substr($url, 7);
    if ($host != "mjsec.cstec.kr" && !preg_match("/\.mjsec\.cstec\.kr$/", $host)) 
        die("no hack");
    ...
    if ($parse_url["host"] !== "mjsec.cstec.kr" && !preg_match("/\.mjsec\.cstec\.kr$/", $parse_url["host"])) 
        die("no hack2");
    ...
    $body .= "Host: internal-ping.mjsec.local\r\n";
